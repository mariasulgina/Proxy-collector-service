...
// Daemon — /daemon/status может вернуть строку или объект
export const pingCollector = () => request('/collector/ping')

export const getDaemonStatus = async () => {
  const raw = await request('/daemon/status')
  // Бэк возвращает сырую строку из Collector — парсим безопасно
  if (typeof raw === 'string') {
    try { return JSON.parse(raw) } catch { return { isRunning: false, raw } }
  }
  return raw
}
