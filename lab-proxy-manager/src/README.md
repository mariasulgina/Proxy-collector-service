...
| --- | --- | --- |
| 1 | **Пакетный менеджер** | Загрузка, фильтрация и группировка прокси по провайдерам (Oxylabs, SOAX). |
| 2 | **Фоновый Демон (Worker)** | `BackgroundService` на базе .NET 9, оптимизированный под высокую нагрузку на память. |
| 3 | **Валидатор (Чекинг)** | Асинхронный пинг целевых узлов, проверка кодов ответов и доступности сети. |
